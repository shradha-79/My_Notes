...
const Notes = (props) => {
    ...
    //used to navigate to any particular path
    let history = useNavigate();
    useEffect(() => {
        if(localStorage.getItem('token')){
            //get all the notes if user is logged in
            getNotes()
        }
        else{
            //can't navigate to home without logging in
            history("/login")
        }
    ...
    return <div>
        ...
        <div className="row my-3">
            ...
            <div className="container mx-2">
                {notes.length===0 && 'NoNotes to display'}
            </div>
            {notes.map((note) => {
